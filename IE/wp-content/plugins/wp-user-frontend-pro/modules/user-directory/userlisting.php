...
class WPUF_User_Listing {
    ...
    function admin_menu_top() {
        ...
        // add_action( "load-$profile_forms_page", array( $this, 'profile_styles' ) );
    }
    ...
    function wpuf_user_listing_init( $atts ) {
        extract( shortcode_atts( array('role' => 'all', 'per_page' => '6'), $atts ) );
    ...
    function list_content_single( $user ) {
        ...
        if ( 'list' == $user_listing_template ) {
            ...
            foreach($unique_meta as $meta_key => $label ) {
                ?>
                <td><?php echo $user->$meta_key; ?></td>
                <?php
            }
            ?>

            <td><a href="<?php echo $this->get_user_link( $user->ID ); ?>">View Profile</a></td>
    ...
    function get_all_user( $user_role ) {
        ...
        if ( isset( $_GET['wpuf_user_search'] ) && !empty( $_GET['search_field'] ) ) {
            ...
            if ( isset( $_GET['search_by'] ) && 'all' != $_GET['search_by'] && in_array( $_GET['search_by'], array( 'user_login', 'user_nicename', 'user_email', 'display_name', 'user_url' ) ) ) {
        ...
        foreach ( $user_results as $user_obj ) {
            ...
            //filter user role
            if ( $user_role != 'all' && $role != $user_role ) {
                continue;
            }
            ...
            foreach ($profile_fields['fields'] as $key => $field) {
                ...
                switch ($field['type']) {
                    case 'meta':
                        ...
                        $value = trim( $userdata->{$field['meta']} );
                        ?>
