...
if( isset( $_POST['remove_api'] ) && wp_verify_nonce( $_POST['mailchimp_remove_nonce'], 'wpuf_mailchimp_api_key_remove_nonce' ) ) {
    ...
    $success = 'Successfully removed your API key';
}
...
<div class="postbox">
    <h3 style="padding:10px 15px"><?php _e( 'Mailchimp','wpuf' ); ?></h3>
    <div class="inside wpuf_mc_wrapper">
        <div class="main">
        <?php if ( get_option( 'wpuf_mailchimp_api_key' )): ?>

            <p id="wpuf_mc_api"><?php _e('Your Mailchimp API key is :','wpuf'); ?> <?php echo get_option( 'wpuf_mailchimp_api_key' ); ?></p>
            ...
            <form action="" method="post" id="wpuf_mc_api_remove_action">
                ...
                <input type="submit" name="remove_api" class="button remove_api_class" value="<?php _e('Remove','wpuf'); ?>" onclick="return removeApiConfirm()">
            </form>
            ...
            <form action="" method="post" id="wpuf_mc_api_refresh_action">
                ...
                <input type="submit" name="refresh_mailchimp_lists" class="button button-primary refresh_api_class" value="<?php _e('Refresh','wpuf'); ?>">
            </form>

        <?php else: ?>

            <form action="" method="post" style="margin-top: 20px;" id="wpuf_mailchimp_api_form">
                <p class="help"><?php _e( 'Please insert your Mailchimp API key.','wpuf' ) ?> <a target="_blank" href="http://admin.mailchimp.com/account/api"><?php _e( 'Get your API key here.', 'wpuf' ); ?></a></p>
                ...
                <input type="submit" class="button button-primary" name="connect_mailchimp" value="<?php _e( 'Connect','wpuf' ) ?>">
            </form>
...
<table class="widefat meta" style="margin-top: 20px;">
    <thead>
        <tr>
            <th scope="col"><?php _e( 'SL. No.', 'wpuf' ); ?></th>
            <th scope="col"><?php _e( 'List Name', 'wpuf' ); ?></th>
        </tr>
    </thead>
    <tbody>
    ...
    <?php else: ?>
            ...
            <?php if ( get_option( 'wpuf_mailchimp_api_key' ) ): ?>
                <?php list(, $datacentre) = explode('-', get_option( 'wpuf_mailchimp_api_key' )); ?>
                <td>
                    <?php _e( 'No List Found. ', 'wpuf' ) ?><a href="https://<?php echo $datacentre; ?>.admin.mailchimp.com/lists/" target="_blank"><?php _e( 'Create your list here', 'wpuf' ) ?></a>
                </td>
            <?php else: ?>
                <td>
                    <?php _e( 'You are not connected with mailchimp. Insert your API key if not ', 'wpuf' )  ?><a target="_blank" href="http://admin.mailchimp.com/account/api"><?php _e( 'Get your API key here.', 'wpuf' ); ?></a>
                </td>
            <?php endif ?>
