...
<div class="col-sm-12 col-md-12 col-lg-8">
	...
	<footer class="entry-footer white-block">
		...
		<?php //understrap_entry_footer(); 
			include $_SERVER['DOCUMENT_ROOT'].'/ie/wp-content/themes/adoptAPetChild/inc/pet-details/dog-details.php';
		?>
        </div>
	</footer><!-- .entry-footer -->
    ...
    $related_posts = get_related_posts( 'breed', ['posts_per_page' => 3, 'orderby' => 'rand'] );
    if ( $related_posts ) {
        foreach ( $related_posts as $post ) {
            $related_images = get_post_meta( get_the_ID(), 'images_dogs', true );
            $related_images_thumb = wp_get_attachment_image_src( $related_images, 'medium' );
            
            setup_postdata( $post ); ?>
            
            <div class="col-lg-4 col-md-4 col-sm-12 col-12 float-left no-gutters related desktop">
                <div class="col-12 white-block">
                    <a href="<?php the_permalink() ;?>" title="<?php the_title();?> | TAGS - Ireland's largest Pet Adoption Website">
                        <div class="white-block-media">
                            <img src="<?php echo $related_images_thumb[0];?>" />
                        </div>
                        <div class="white-block-content">
                            <h2><?php the_title();?></h2>
                            
                            <p><?php if ( $string = get_the_author_meta('county') ) {
                                    echo $string;
                                }
                            ?></p>
