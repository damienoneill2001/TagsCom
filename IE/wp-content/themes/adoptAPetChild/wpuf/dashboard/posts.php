...
<?php if ( wpuf_get_option( 'show_post_count', 'wpuf_dashboard', 'on' ) == 'on' ) { ?>
    <div class="white-block post_count"><?php printf( __( 'You have <span>%d</span> active pet ads', 'wpuf' ), $dashboard_query->found_posts, $post_type_obj->label ); ?></div>
<?php } ?>
   ...
   <!-- <table class="items-table <?php echo $post_type; ?>" cellpadding="0" cellspacing="0">
        ...
        <tbody>-->
            ...
            while ( $dashboard_query->have_posts() ) {
                ...
                ?>
                 <div class="white-block petResults userAccount float-left col-sm-12 col-12 col-md-12 col-lg-12 no-gutters <?php if ( !$show_link ): { ?>expired <?php } endif; ?>" style="">
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-4 col-sm-4 col-4 float-left">
                        <div class="white-block-media">
                            ...
                            <?php endif; ?>
                        <?php $imagesDogs = get_post_meta( $post->ID, 'images_dogs', true );
                              $imagesCats = get_post_meta( $post->ID, 'images_cats', true );
                              $imagesOthers = get_post_meta( $post->ID, 'images_other', true );
                        
                        if ($imagesDogs) {
                                    
                                        $thumb = wp_get_attachment_image( $imagesDogs, 'thumbnail' );
                                        $full_size = get_the_permalink();
                            ...
                            //echo $thumb;
                                        ...
                                        $thumb = wp_get_attachment_image( $imagesCats, 'thumbnail' );
                                        $full_size = get_the_permalink();

                            printf( '<a href="%s">%s</a>', $full_size, $thumb );
                            
                        } else if ($imagesOthers) {
                                    
                                        $thumb = wp_get_attachment_image( $imagesOthers, 'thumbnail' );
                                        $full_size = get_the_permalink();
                    ...
                    <div class="ad-content entry-content col-xl-9 col-lg-8 col-md-8 col-sm-8 col-8 float-left">
                    ...
                    <h2>
                        <?php if ( !$show_link ) { ?>
                            ...
                            <div class="draft"><?php the_title(); ?></div>
                    ...
                    <div class="userAccountExcerpt">
                    <?php the_excerpt();?>
                    </div>
                    

                    <!--<div class="postStatus">
                        <i class="fa fa-check-square-o" aria-hidden="true"></i> <?php wpuf_show_post_status( $post->post_status ) ?>
                    </div>-->
                    ...
                    if ( $charging_enabled == 'yes' ) {
                        ...
                        <div>
                            <?php if ( $post->post_status == 'pending' && $order_id ) { ?>
                                <a href="<?php echo trailingslashit( get_permalink( wpuf_get_option( 'payment_page', 'wpuf_payment' ) ) ); ?>?action=wpuf_pay&type=post&post_id=<?php echo $post->ID; ?>"><?php _e( 'Renew Ad', 'wpuf' ); ?></a>
                            <?php } ?>
                        </div>
                    <?php } ?>
                    ...
                    <div class="editLink">
                        ...
                        if ( wpuf_get_option( 'enable_post_edit', 'wpuf_dashboard', 'yes' ) == 'yes' ) {
                            ...
                            if ( $post->post_status == 'pending' && $disable_pending_edit == 'on' ) {
                                ...
                                ?>
                                <a href="<?php echo wp_nonce_url( $url, 'wpuf_edit' ); ?>"><i class="fa fa-edit" aria-hidden="true"></i> <?php _e( 'Edit', 'wpuf' ); ?></a> 
                                <?php
                            }
                        }
                        ?>
                    ...
                    <div class="deleteLink">
                        ...
                        if ( wpuf_get_option( 'enable_post_del', 'wpuf_dashboard', 'yes' ) == 'yes' ) {
                            ...
                            ?>
                            <a href="<?php echo wp_nonce_url( $del_url, 'wpuf_del' ) ?>" onclick="return confirm('<?php echo $message ?>');"><span style="color: red;"><i class="fa fa-remove" aria-hidden="true"></i>  <?php _e( 'Delete', 'wpuf' ); ?></span></a>
                        <?php } ?>
