   ...
   <!-- <table class="items-table <?php echo $post_type; ?>" cellpadding="0" cellspacing="0">
        ...
        <tbody>-->
            ...
            while ( $dashboard_query->have_posts() ) {
                ...
                ?>
                 <div class="row no-gutters white-block ad-box ad-box-alt <?php if ( !$show_link ): { ?>expired <?php } endif; ?>" style="padding:20px 20px;">
                    <div class="ad-image col-md-3">
                        <?php $imagesDogs = get_post_meta( $post->ID, 'images_dogs' );
                              $imagesCats = get_post_meta( $post->ID, 'images_cats' );
                              $imagesOthers = get_post_meta( $post->ID, 'images_other' );

                                if ($imagesDogs) {
                                    foreach ($imagesDogs as $attachment_id) {
                            ...
                            printf( '<a href="%s">%s</a>', $full_size, $thumb );
                                    ...
                                    foreach ($imagesCats as $attachment_id) {
                                        $thumb = wp_get_attachment_image( $attachment_id, 'thumbnail' );
                                        $full_size = wp_get_attachment_url( $attachment_id );
                            ...
                            printf( '<a href="%s">%s</a>', $full_size, $thumb );
                                    ...
                                    foreach ($imagesOthers as $attachment_id) {
                                        $thumb = wp_get_attachment_image( $attachment_id, 'thumbnail' );
                                        $full_size = wp_get_attachment_url( $attachment_id );
